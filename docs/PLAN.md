...
## 9. 잠재 이슈 / 결정 보류 항목
- **인증 영속성 vs 평문 토큰** (결정됨 — `TUNNEL_PERSIST_AUTH=1` default): VS Code CLI는 file-keychain fallback에서도 컨테이너 인스턴스 keyring 기반으로 토큰을 암호화 → named volume의 token.json이 새 컨테이너에서 복호화 불가. 본 이미지는 `VSCODE_CLI_DISABLE_KEYCHAIN_ENCRYPT=1`을 entrypoint에서 자동 설정해 평문 JSON으로 저장한다 (디렉터리 0700). 운영 현실(named volume = 영속성)과 맞추기 위한 의도적 선택. 멀티유저 호스트/공유 볼륨 시나리오에선 `-e TUNNEL_PERSIST_AUTH=0`으로 opt-out, 또는 외부 secret store(docker/K8s)에서 `VSCODE_CLI_ACCESS_TOKEN` 주입.
